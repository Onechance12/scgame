...
const Entities = (() => {
      ...
      if (this.state === S.HUNT) {
        ...
        if (this.pause > 0) {
          ...
          if (arrived) {
            ...
            const stareChance = this.kind === 'nurse' ? 0.6 : this.kind === 'child' ? 0.5 : 0.3;
  ...
  function spawnAll(world, opts) {
    ...
    p = R(0, 'nursery'); add({ name: 'The Child', kind: 'child', floor: 0, x: p.x, y: p.y, speed: 1.5, huntSpeed: 2.7, hearing: 8, sight: 6, wakeHour: 6, den: ['nursery', 'xray', 'storage'] });
    ...
    p = R(2, 'ward');  add({ name: 'Night Nurse', kind: 'nurse', floor: 2, x: p.x, y: p.y, speed: 1.8, huntSpeed: 3.1, hearing: 6, sight: 8, wakeHour: 2, den: ['ward', 'station', 'room207', 'maternity', 'bath'] });
